...
class Sampler():
    def __init__(self, conditional_vecto_field_model_path : str, config: SamplerConfig): 
        ...
        self.guidance_embedding_model = MergingResnet18(num_images=cvf_config.past_horizont, input_channels=cvf_config.img_channels, embedding_size=cvf_config.visual_embedding_dim, num_groups=cvf_config.visual_embedding_dim)
        self.guidance_embedding_model.load_state_dict(state_dict['guidance_embedding_model_state_dict'], )
    ...
    def sample(self, guidance_images: torch.Tensor) -> torch.Tensor:
        assert guidance_images.shape[1] == self.guidance_embedding_model.num_images == self.cvf_config.past_horizont, f"Past horizont mismatch {guidance_images.shape[1]} vs {self.guidance_embedding_model.num_images} vs {self.cvf_config.past_horizont}"

        batch_size = guidance_images.shape[0]

        with torch.no_grad():
            guidance_embedding = self.guidance_embedding_model(guidance_images)
            x = torch.randn((batch_size, self.cvf_config.future_horizont, self.cvf_config.sequence_features), device=guidance_images.device)
            t = 0.0

            for i in range(self.num_steps):
                predicted_guided_vecor_field = self.unet(x, t*torch.ones(batch_size, device=guidance_images.device), guidance_embedding)
                ...
                predicted_vector_field = (1 - self.config.cfg_strength) * predicted_null_vecotr_field + self.config.cfg_strength * predicted_guided_vecor_field

                x += ((1 + self.sigma**2/(2 * self.a(t)))*predicted_vector_field - (self.sigma**2 * self.b(t))/(2*self.a(t)) * x) * self.dt + self.sigma * torch.sqrt(self.dt) * torch.randn_like(x)
    ...
    def a(self, t):
        return self.beta(t)**2 * self.alpha_dot(t) / self.alpha(t) - self.beta(t) * self.beta_dot(t)
...
if __name__ == "__main__":
    ...
    dataset = CarDataset("./dataset/", device=device, future_horizont=2, past_horizont=2)
